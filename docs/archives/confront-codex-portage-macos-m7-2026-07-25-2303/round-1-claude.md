...
## Ce sur quoi je veux être challengé en priorité

Ce plan repose sur cinq affirmations. Les trois premières sont des faits de
plateforme que je n'ai **pas pu observer** — il n'y a pas de Mac sur cette machine,
et M8 a montré ce que valent les hypothèses non vérifiées (un `Segmentation fault` à
la première exécution native, invisible en test mocké). Va les vérifier plutôt que me
croire.

1. **« Un bundle assemblé localement ne reçoit jamais l'attribut de quarantaine. »**
   Mon raisonnement : `com.apple.quarantine` est posé par l'agent qui télécharge
   (LaunchServices, le navigateur, l'installateur de cask), pas par le système de
   fichiers. Un dossier écrit par un processus Python local n'en reçoit donc pas.
   Est-ce vrai dans tous les cas ? Y a-t-il un chemin par lequel macOS marquerait
   quand même une `.app` neuve — provenance héritée du dossier parent, `xattr`
   propagé depuis l'archive Homebrew, politique plus récente sur Sequoia/Tahoe ?

2. **« Les autorisations TCC suivent l'application qui lance. »** Donc une `.app`
   qui démarre Python recevrait les fenêtres à son nom, là où aujourd'hui c'est
   Terminal. Mon raisonnement s'appuie sur la notion de *responsible process*.
   Mais : est-ce que ça tient quand le `CFBundleExecutable` est un **script shell**
   qui fait `exec` sur un interpréteur situé **hors du bundle** (dans le préfixe
   Homebrew) ? Le `exec` remplace l'image du processus — le responsable
   reste-t-il le bundle, ou devient-il le binaire Python ? C'est le point dont
   dépend tout l'intérêt du lot : s'il tombe, on a construit un bundle pour rien
   et les autorisations restent attribuées au mauvais responsable.

3. **« Le bundle doit être stable octet pour octet, sinon le `cdhash` change et
   macOS oublie les autorisations. »** Et la contre-mesure que je propose — ne pas
   mettre la version d'Aparté dans le bundle, garder le chemin `opt` stable de
   Homebrew — est-elle **suffisante** ? Est-ce que le `cdhash` est bien ce qui sert
   d'identité pour une signature ad-hoc, ou est-ce que TCC retombe sur
   `CFBundleIdentifier` + chemin, auquel cas l'invariant est inutilement
   contraignant ? Les deux erreurs coûtent : trop contraignant, on s'interdit
   d'écrire la version quelque part d'utile ; pas assez, l'utilisateur réautorise
   à chaque mise à jour et le lot rate sa cible.

4. **Le LaunchAgent qui passe par `/usr/bin/open -a`** plutôt que d'exécuter le
   lanceur directement, pour que LaunchServices démarre l'application et qu'elle
   reste responsable. Est-ce le bon mécanisme en 2026, ou faut-il `SMAppService`
   (macOS 13+) ? Que se passe-t-il si `open` rend la main immédiatement — launchd
   considère-t-il le service comme mort et le relance-t-il en boucle ?

5. **Le découpage M7a–M7h.** Est-ce que l'ordre expose les risques assez tôt ? Le
   point 2 ci-dessus est le pari central du lot et il ne se vérifie qu'à M7c, sur
   un vrai Mac. Faut-il un lot zéro qui ne fait que le prouver — un bundle minimal,
   une fenêtre d'autorisation, une capture d'écran — avant d'écrire le reste ?

## Ce que je ne veux pas rouvrir

- **Le cask est mort**, pour les raisons datées en tête de ce document. Si tu penses
  le contraire, il faut un fait, pas une préférence.
- **Le modèle Whisper ne s'embarque pas** : tranché par le propriétaire du projet.
- **Le comportement Linux ne bouge pas.** Toute proposition qui touche au chemin
  Linux existant est hors périmètre.
- **Pas de compte développeur Apple** (99 USD/an, et il faudrait un Mac pour signer
  chaque version — il n'y en a pas).
